...
async def generate_content_highlights(
    ...
    lines = [
        "## 真实对话候选片段",
        "请只基于下面片段提取 3-5 个内容亮点。每个亮点必须有一个判断和 2-4 行证据。",
...
async def call_llm_multi(
    ...
    # Call 6: Content highlights backed by real dialogue snippets.
    highlights = await _run_with_progress(
        "content_highlights",
        generate_content_highlights(highlight_windows or [], report_type),
    )
    results["content_highlights"] = highlights
